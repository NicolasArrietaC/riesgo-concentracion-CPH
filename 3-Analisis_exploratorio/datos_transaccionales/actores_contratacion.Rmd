...
Por parte de los contratistas, no se observa un patrón claro de la relación de los contratos y el número de entidades, como sucede en el anterior caso con las entidades. 


por lo que se refuerza la idea de las diferencias de las características entre entidades y contratistas.
...
temp <- contratos %>% 
  mutate(trimestre = quarter(fecha_firma_contrato, with_year = T)) %>%
  group_by(trimestre, id_contratista_std) %>% 
  summarise(n = n(),
            v = sum(valor_total_con_adiciones) / 1e6) %>% 
