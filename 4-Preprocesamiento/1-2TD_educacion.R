...
ind_edu <- ind_edu %>% 
 ...
 summarise(
  ...
  ine_coberturaNeta_sd = sd(ine_coberturaNeta, na.rm = T),
  # Tasa de desrcion
  ine_tasaDesercion_m = mean(ine_tasaDesercion, na.rm = T),
  ine_tasaDesercion_sd = sd(ine_tasaDesercion, na.rm = T),
  # Tasa repitencia
  ine_tasaRepitencia_m = mean(ine_tasaRepitencia, na.rm = T),
  ine_tasaRepitencia_sd = sd(ine_tasaRepitencia, na.rm = T),
  # Tasa analfabetistmo
  ...
  ine_pruebaLecturaMatematicas_sd = sd(ine_pruebaLecturaMatematicas, na.rm=T),
  # Prueba de lectura
  ine_pruebaLecturaCritica_m = mean(ine_pruebaLecturaCritica, na.rm = T),
  ine_pruebaLecturaCritica_sd = sd(ine_pruebaLecturaCritica, na.rm = T)
 )

# 4. Escritura de datos ----
direccion <- '1-2-Datasets/terridata/'
write_csv(ind_edu, paste0(direccion, '1-2A_terr_indsel_educacion.csv'))
