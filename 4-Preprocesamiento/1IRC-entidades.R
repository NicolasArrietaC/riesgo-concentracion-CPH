...
ind_ent <- contratos %>% group_by(nit_entidad) %>% 
 summarise(num_contratos = n(),
           ...
           num_contratistas_dif = n_distinct(id_contratista)) %>% 
...
temp <- contratos %>% group_by(nit_entidad, id_contratista) %>% 
 summarise(num_contratos = n(),
           val_contratos = sum(valor_total_con_adiciones)) %>% 
...
# Salida: pganadoras: porcentaje de empresas ganadoras por cada 100 contratos*
...
temp <- contratos %>% 
 group_by(nit_entidad, id_contratista) %>% count() %>% 
 ungroup(id_contratista) %>% 
 mutate(Pi = n / sum(n)) %>%
